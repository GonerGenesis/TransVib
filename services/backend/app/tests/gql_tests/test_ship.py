...
async def test_get_ship(http_client):
    ...
    assert float(json["data"]["getShip"]["frames"][0]["frameSegments"][0]["thick"]) == pytest.approx(float(segment.thick))
...
async def test_update_ship(http_client):
    ...
    mutation = """
                mutation UpdateShip {{
                    updateShip(shipId: {} ship: {{title: "moep", description: "miep", authorId: 1}}){{
                    title
                    description
                    id
                    }}
